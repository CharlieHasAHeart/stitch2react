...
The default blueprint path remains deterministic after first-pass generation. Stitch HTML generation now adds Chrome headless direct CDP runtime validation before validated artifact persistence. React remains a future direction.
...
flowchart TD
  ...
  L --> M["Chrome headless direct CDP runtime validation"]
...
```mermaid
flowchart TD
  A["stitch_html artifact"] --> B["Open page in Chrome headless via direct CDP"]
...
- Chrome headless direct CDP runtime validation checks real click behavior, rendered page health, console/runtime errors, and navigation behavior.
...
flowchart TD
  ...
  D --> E["Chrome headless direct CDP runtime validation"]
  ...
  G -->|Not fixable| K["Fail with diagnostics"]
```
